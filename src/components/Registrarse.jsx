...
export default function Registrarse({ setUsuarioLogueado }) {
  ...
  const manejarRegistro = (e) => {
    e.preventDefault();
    if (!nombre || !correo || !contrasena) return alert("Por favor, rellena todos los campos.");
    ...
    <div style={contenedorFondo}>
      ...
      <div style={cajaBlancaFormulario}>
        ...
        <form onSubmit={manejarRegistro} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
          ...
          <button type="submit" style={botonAccionPrincipal}>REGISTRARSE</button>
        </form>

        <div style={enlaceIntercambio}>
          ¿Ya tienes una cuenta? <span onClick={() => navigate('/autenticacion')} style={textoResaltadoEnlace}>Inicia sesión aquí</span>
        </div>
      </div>
    </div>
  );
